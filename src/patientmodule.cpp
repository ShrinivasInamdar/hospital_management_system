...
vector<string> getMedicalHistory(sqlite3* db, int patientId);
...
public:
    ...
    void set_status(int ps){status=ps;}
    void set_doc(int pd){doctorID = pd;}

   void fetchMedicalHistoryFromDB(sqlite3* db) {
        medical_history = getMedicalHistory(db, patientId);
    }
